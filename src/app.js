...
import VideosPromocion from "./componentes/VideosPromocion";
...
function App() {
  ...
  const mostrarManualesGenericos = (categoria, opcion) => {
    ...
    seleccion = filtrarKey(
      tmpArray,
      "clave",
      opcion,
      "mostrarManualesGenericos"
    )[0];
  ...
  const mostrarVideosPromocion = (opcion) => {    
    setVisor(<VideosPromocion  opcion={opcion} />);
  };
  ...
  const handleMostrarListaManuales = (e) => {
    ...
    if (item) {
      ...
      let indice = parseInt(e.currentTarget.id);
      console.log(
        " Mostrando lista de manuales ************** opcion sub",
        indice
      );
      seleccion = aplicativos[indice];
      console.log("Seleccion --->", seleccion);
